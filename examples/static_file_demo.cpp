/**
 * EvoAudioSDK 静态文件识别示例
 *
 * Usage:
 *   ./stt_test [audio_file] [model_dir]
 *
 * Examples:
 *   ./stt_test                              # 使用默认测试文件
 *   ./stt_test ~/test.wav
 *   ./stt_test ~/test.wav ~/.cache/sensevoice
 */
...
void printUsage(const char* program) {
    ...
    std::cout << "              Default: ~/.cache/sensevoice" << std::endl;
    std::cout << std::endl;
...
void TestFileRecognition(const std::string& audio_file, const std::string& model_dir) {
    std::cout << "========================================" << std::endl;
    std::cout << "    EvoAudioSDK 文件识别测试" << std::endl;
    std::cout << "========================================" << std::endl;
    std::cout << std::endl;
    ...
    Evo::AsrConfig config = Evo::AsrConfig::SenseVoice(model_dir);
    ...
    if (result && !result->IsEmpty()) {
        ...
        Evo::Sentence sentence = result->GetSentence();
...
int main(int argc, char* argv[]) {
    ...
    std::string model_dir = "~/.cache/sensevoice";  // 空则使用默认路径
