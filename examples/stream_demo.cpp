/**
 * EvoAudioSDK 流式识别示例 (定时 Flush)
 *
 * 使用 EvoAudio::AudioCapture 采集麦克风，每 3 秒自动 Flush 触发识别。
 * 简化演示版本，无 VAD。
 *
 * 音频流程: 48kHz stereo → 重采样 → 16kHz mono → ASR
 *
 * Usage:
 *   ./stream_demo [options] [device_index] [total_seconds]
 *
 * Examples:
 *   ./stream_demo                # 默认设备，总时长 30 秒
 *   ./stream_demo -l             # 列出设备
 *   ./stream_demo 0 60           # 设备 0，总时长 60 秒
 *   ./stream_demo -i 5           # 每 5 秒 flush 一次
 */
...
class StreamCallback : public Evo::AsrEngineCallback {
public:
    ...
    void OnEvent(std::shared_ptr<Evo::RecognitionResult> result) override {
        if (!result) return;
    ...
    void OnError(std::shared_ptr<Evo::RecognitionResult> result) override {
        if (result) {
    ...
    // 获取最后的完整结果
    std::shared_ptr<Evo::RecognitionResult> getLastResult() const {
        std::lock_guard<std::mutex> lock(mutex_);
        return last_result_;
    }
...
private:
    ...
    std::shared_ptr<Evo::RecognitionResult> last_result_;
...
void listDevices() {
    ...
    auto devices = EvoAudio::AudioCapture::ListDevices();
...
public:
    ...
    void setChannels(int channels) {
        ...
        if (channels != channels_) {
            channels_ = channels;
            resampler_ = EvoAudio::Resampler(makeResamplerConfig(channels));
            resampler_.initialize();
        }
    }
...
private:
    static EvoAudio::Resampler::Config makeResamplerConfig(int channels) {
        EvoAudio::Resampler::Config config;
        config.input_sample_rate = INPUT_SAMPLE_RATE;
        config.output_sample_rate = OUTPUT_SAMPLE_RATE;
        config.channels = 1;  // 已经混合为单声道
        config.method = EvoAudio::ResampleMethod::LINEAR_DOWNSAMPLE;
...
int main(int argc, char* argv[]) {
    ...
    std::cout << "========================================" << std::endl;
    std::cout << "  EvoAudioSDK 流式识别 (定时 Flush)" << std::endl;
    std::cout << "========================================" << std::endl;
    std::cout << std::endl;
    ...
    Evo::AsrConfig config = Evo::AsrConfig::SenseVoice();  // 使用默认模型路径
    config.language = "zh";
    config.punctuation = true;

    auto asrEngine = std::make_shared<Evo::AsrEngine>(config);
    ...
    // 创建音频采集器
    EvoAudio::AudioCapture capture(device_index);
