/**
 * Evo ASR Engine Adapter Implementation
 *
 * 适配层实现，将 asr::ASREngine 封装为 Evo::AsrEngine 接口。
 */
...
public:
    ...
    void onResult(const asr::RecognitionResult& result) override {
        ...
        if (callback_) {
            auto wrapped_result = std::make_shared<RecognitionResult>();
            wrapped_result->setFromInternal(result);
            callback_->OnEvent(wrapped_result);
        }
    }
...
AsrEngine::AsrEngine(const std::string& engine, const std::string& model_dir)
    ...
    if (engine == "sensevoice" || engine.empty()) {
        // 使用 SenseVoice 后端
        std::string dir = model_dir.empty() ? "~/.cache/sensevoice" : model_dir;
...
AsrEngine::AsrEngine(const AsrConfig& config)
    ...
    if (config.engine == "sensevoice" || config.engine.empty()) {
        std::string dir =
            config.model_dir.empty() ? "~/.cache/sensevoice" : config.model_dir;
